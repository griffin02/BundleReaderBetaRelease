...
def _read_complex_constants(parser, complex_constants):
	...
	while i < parser.complex_constants_count:
		...
		if constant_type >= BCDUMP_KGC_STR:
			...
			##bytes = parser.stream.read_bytes(length)
			if not ljd.config.latin_1_config.use_latin_1:
				complex_constants.append(string.decode("utf-8", "backslashreplace"))
			else:
				complex_constants.append(string.decode("Latin-1"))
			##complex_constants.append(parser.stream.decode_uleb128(bytes, length))
...
def _read_table_item(parser):
	...
	if data_type >= BCDUMP_KTAB_STR:
		...
		if not ljd.config.latin_1_config.use_latin_1:
			...
			return parser.stream.read_bytes(length).decode("Latin-1")
